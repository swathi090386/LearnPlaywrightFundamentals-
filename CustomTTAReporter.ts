...
class CustomTTAReporter implements Reporter {
    ...
    private associateLogsWithSteps(_test: TestCase, result: TestResult, testSteps: StepData[]): void {
        ...
        for (const attachment of result.attachments) {
            ...
            if (logMatch && attachment.contentType === 'text/plain' && logMatch[1]) {
        ...
        for (let logIndex = 0; logIndex < allLogs.length; logIndex++) {
            ...
            for (let stepIndex = 0; stepIndex < testSteps.length; stepIndex++) {
                const pattern = stepTitlePatterns[stepIndex];
                const step = testSteps[stepIndex];
                if (!pattern || !step) {
                    continue;
                }
                if (pattern.test(log)) {
                    step.consoleLogs = step.consoleLogs || [];
                    step.consoleLogs.push(log);
        ...
        if (unassignedLogs.length > 0 && testSteps.length > 0) {
            const stepsNeedingLogs = testSteps.filter(s => (s.consoleLogs?.length || 0) === 0);
            ...
            if (stepsNeedingLogs.length > 0) {
                ...
                for (let stepIdx = 0; stepIdx < testSteps.length && logIdx < unassignedLogs.length; stepIdx++) {
                    ...
                    if ((step.consoleLogs?.length || 0) === 0) {
                        step.consoleLogs = step.consoleLogs || [];
                        for (let i = 0; i < logsForThisStep; i++) {
                            step.consoleLogs.push(unassignedLogs[logIdx++] as string);
                        }
                    }
                }
            } else {
                ...
                if (firstStep) {
                    firstStep.consoleLogs = firstStep.consoleLogs || [];
                    firstStep.consoleLogs.push(...(unassignedLogs as string[]));
                }
            }
        }
    }
